...
| エラーレベル | 動作 | レポート表示 |
|-------------|------|-------------|
| E1 | その試合のレポート生成をスキップ、他試合は継続 | 選外試合リストに記載 |
| E2 | 欠損項目を「不明」として表示、レポート生成を継続 | エラーステータス欄に表示 |
| E3 | 警告ログを出力、レポート生成を継続 | エラーステータス欄に表示 |

#### 機能別エラー影響

| 機能 | エラー時の動作 |
|------|---------------|
| Match | E1 → 全体停止、E2 → 対象試合スキップ |
| Facts | E2 → スタメン「不明」、フォーメーション「不明」 |
| News | E3 → 「記事が見つかりませんでした」 |
| YouTube | E3 → 動画セクションを省略 |
| Report | E2/E3 → エラーステータスを表示して生成継続 |
...
> **Note**: APIクォータの詳細は [api_management.md](../02_design/api_management.md) を参照
