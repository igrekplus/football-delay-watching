...
本システムは、ユーザーが試合結果（スコア・勝敗・得点者など）を一切知らずに録画・配信でサッカー観戦を楽しめるようにすることを目的とする。そのために、試合前の事実情報とニュース・プレビューのみを用い、3時間ごとの定期実行で対象試合のネタバレを含まない観戦ガイドを自動生成する。
...
| No. | 機能名 | 概要 | 詳細要件 |
|-----|--------|------|----------|
| 1 | 試合抽出・選定 (Match) | 対象試合を抽出し、優先順位で最大5試合選定 | [3.1](#31-試合抽出選定-match) |
...
| 5 | レポート生成 (Report) | レポートコンテンツ出力（選外試合含む） | [3.4](#34-レポート生成-report) |
| 6 | 配信 (Delivery) | HTML変換・メール送信 | [3.5](#35-配信-delivery) |
| 7 | カレンダー (Calendar) | リーグ別の試合一覧表示とレポート導線提供 | [3.6](#36-カレンダー-calendar) |
| 8 | キャッシュウォーミング (CacheWarming) | 上位チームデータの事前取得 | [3.7](#37-キャッシュウォーミング-cachewarming) |
...
3時間ごとの定期実行で、現在時刻を基準とした対象ウィンドウ内の試合を抽出する。

- 実行日時：3時間ごと（GitHub Actions）
- 対象期間：キックオフ1時間前〜キックオフ24時間後（再処理対象を含む）
- 対象リーグ：
  - プレミアリーグ（EPL）
  - UEFA チャンピオンズリーグ（CL）
  - ラ・リーガ（LALIGA）
...
抽出された全試合の中から、レポート対象とする最大 5 試合を選定する。

**ランク定義**

| ランク | 条件 | 説明 |
|--------|------|------|
| Sランク | 優先クラブ対戦 | 重点クラブを含む試合を最優先で採用 |
| Aランク | 注目クラブ対戦 | 準重点クラブ・注目カードを優先採用 |
| None | 通常カード | 上記条件に該当しない試合 |

**枠配分ロジック**
1. Sランク → Aランク → None の順に採用
2. 同一ランク内では大会優先度（CL > LALIGA > EPL > COPA > FA > EFL）で並べる
3. 高ランク試合が 5 件未満の場合は None を補充し、常に最大 5 試合まで出力する
...
試合前時点の移籍市場情報（獲得・放出・噂）を、チームごとに生成する。

- 対象：各試合のホーム/アウェイ両チーム
- データ源：Gemini Grounding
- 制約：
  - 試合日時点より未来の情報を含めない
  - 信頼性の低いソースは優先しない
  - 試合結果・ネタバレ情報を含めない
...
1. 本日の対象試合サマリー
2. 試合別レポート（最大 5 試合）
   - 基本情報、ニュース要約、戦術プレビュー、移籍情報、エラーステータス
3. 選外試合リスト（選外理由を記録）
...
**実装**: `CalendarGenerator`

リーグごとの試合日程を一覧表示し、各試合のレポートへ遷移できる画面を提供する。

- 目的：
  - リーグ単位で試合を俯瞰し、観戦計画を立てやすくする
  - 既存レポートへの遷移導線を提供する
- 要件：
  - 6リーグ（EPL / CL / LALIGA / FA / COPA / EFL）を横断表示
  - 4週間（先週〜再来週）の試合を週単位で表示
  - JST/UTCのキックオフ時刻を併記
  - レポート生成済み試合はレポートリンクを表示
  - 試合詳細に実況情報（解説/実況）を表示できる

### 3.7 キャッシュウォーミング (CacheWarming)
