...
| 目的 | 説明 |
|------|------|
| **クォータ節約** | API-Football の日次上限を効率的に活用 |
...
> 詳細は [実行モード設計書](../04_operations/execution_mode.md) を参照。
...
| Team名翻訳 | `team_translation/{team_hash}.json` | `team_translation/1a2b3c4d5e6f7g8h.json` |
...
| **チーム名翻訳** (`team_translation`) | 無期限 | チーム名の表記揺れ補正は低頻度更新 |
...
| **移籍ニュース (Grounding)** | 7日間 | 試合日前後で情報更新される |

> [!NOTE]
> `transfer_news` は `GROUNDING_TTL_DAYS` に明示値がなくても、`LLMClient` の既定値（7日）でTTL管理される。
> `team_translation` は `TeamNameTranslator` 側でキャッシュ読み書きを行い、TTL判定は行わない（実質無期限運用）。
...
- [システム設計書](../02_architecture/system_overview.md) - 全体アーキテクチャ
- [API-Football設計](./api_football.md) - 各エンドポイントのパラメータ
