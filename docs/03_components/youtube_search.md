...
|---------|-------------------|---------|------|---------|
| 記者会見 | `{team} {manager} press conference` | 48時間前〜キックオフ | 英語 | 10 |
| 過去対戦 | `{home} vs {away} highlights` | 730日前〜24時間前 | - | 10 |
| 戦術分析 | `{team} 戦術 分析` | 180日前〜キックオフ | 日本語 | 10 |
| 選手紹介 | `{player} {team} プレー` | 180日前〜キックオフ | 日本語 | 10 |
| 練習風景 | `{team} training` | 168時間前〜キックオフ | 英語 | 10 |
...
**設計判断理由 (Issue #122)**:
現状の実装（`youtube_service.py`）では、フィルタ後の候補確保とシステム実装の簡潔さを優先し、全カテゴリ一律で「10件」の上限を採用している。
（旧設計にあった 1〜6件 という細かな制限は、フィルタ過多による候補不足リスクがあるため採用しない）
