...
レポート生成の候補となる試合データは、`config.TARGET_DATE` を基準に日付単位で取得されます。

```
取得対象: TARGET_DATE と TARGET_DATE+1 の2日分（広めの候補集合）

```
[Target Date - 1 day]  ---  [Target Date]  ---  [Target Date + 1 day]
```

-   **目的**: 3時間ごとのDynamic Schedule実行に対応するため、候補を広めに取得しています。
-   **厳密なフィルタ**: この範囲で取得した試合データに対し、`MatchScheduler` が「キックオフ -1h 〜 +24h」の判定を行い、現在処理すべき試合のみを抽出します。
