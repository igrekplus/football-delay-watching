...
- **出力**: 日本語
- **制約**: フォーメーション・マッチアップにフォーカス、結果言及禁止、AI応答文禁止

---

## 3. 移籍情報 (generate_transfer_news)

### 3.1 目的
チームごとの直近移籍情報（獲得・放出・噂）を試合前情報として生成する。
...
- 入力:
  - `team_name`
  - `match_date`（基準日）
  - `transfer_window_context`（winter/summer/latest）
- 出力構成:
  1. 🆕 獲得・加入 (In)
  2. 👋 放出・退団 (Out)
  3. 💬 噂・動向 (Rumors)
...
- **クライアント**: REST (`GeminiRestClient`)
- **モデル**: `gemini-2.0-flash`
- **入力**: Team名・試合日を基準にGrounding検索
- **出力**: 日本語（固有名詞は英語表記維持）
- **制約**:
  - `match_date` 以降の未来情報を含めない
  - 試合結果言及禁止
  - 信頼性の低いソースを優先しない
