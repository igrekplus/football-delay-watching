...
### 1.3.4 url_id
**意味**：URL単位の識別子（人間が判読可能）。  
**決め方**：`url_id = slugify(domain + path)`  
例：`ja_wikipedia_org__wiki_manchester_city_fc`
...
- 検索計画は **logs/raw_acquisition の実行ログに記録**する（knowledge には保存しない）。
...
## 4.3 クローリング（本文抽出）
### 4.3.1 クローリング指示（重要・粘り強く！）
- **本文領域の特定（最優先）**：`article` / `main` / `div.article-body` 等、本文が格納されている正確なコンテナを特定する。`meta`タグのdescriptionや、一覧ページの抜粋で満足しないこと。
- **改行の維持（可読性）**：
  - 段落（`p`）、見出し（`h1-h6`）、リスト（`li`）、改行（`br`）の前後には必ず改行コード（`\n`）を挿入する。
  - **全テキストを1行に連結してはならない。** 人間が読める形式を維持すること。
- **画像の維持**：ページ内の画像を `[IMAGE] {url} (alt=...)` 形式で本文中の適切な位置に挿入する。
- **ページネーション/全表示の確認**：
  - 記事が分割されている場合（例：「次のページ」「1, 2, 3...」）、**必ず全ページを巡回して本文を結合**する。
  - 「全文を表示」ボタンがある場合は必ずクリックする。
  - URLパラメータ（`?page=2`など）を推測できる場合は積極的に試行する。

### 4.3.2 抽出品質の自己評価（諦めない！）
- **文字数・行数チェック**：
  - 抽出した本文が **20行未満**、または **500文字未満** の場合、それは「失敗」または「要約のみ」の可能性が高い。
  - **再取得プロセス（必須）**:
    1. セレクタを変更する（例: `article` → `main` → `div.content` → `body`内の最大テキストブロック）。
    2. DOM構造を再確認し、別のコンテナを探す。
    3. これを **20行以上取れるまで**、または **可能な限りのセレクタを試すまで** 繰り返す。
- **断片化の回避**：一部しか取れていないと疑われる場合は、親要素を広げて再取得する。

### 4.3.3 高難易度ケースの対応手順（fallback）
**A. 無限スクロール / 動的読み込み**
- コンテンツが増えなくなるまでスクロールを繰り返す（最大5回程度）。
- 遅延読み込み画像（`data-src`等）も考慮する。

**B. DOMが複雑 / 分割**
- 特定のコンテナが見つからない場合、`body`全体からノイズ（ヘッダー、フッター、サイドバー）を除去してテキスト化するアプローチに切り替える。
- 見出し直下のテキストブロックを重点的に拾う。
...
- 有用判定の結果（採用/除外と理由）
- 実際に取得したURL一覧
 - **フィードバック（Playwright/画面操作で苦労した点）**
 - **次回への改善提案（指示書に反映すべき点）**
※ ログは**日本語で出力**し、**計画と結果は1つのmdにまとめる**。
※ 検索計画・候補URL一覧は **knowledge/raw には保存しない**。
...
# 7. 補足・今後に向けて
## 7.1 slugify の手順（url_id 生成）
1. URLのドメインとパスを抽出する。
2. ドメインの `www.` は除去する。
3. ドメインとパスの区切り、およびパス内の区切り文字（スラッシュ、ドット、ハイフン等）をアンダースコア `_` に置換する。
4. 連続するアンダースコアは1つにまとめる。
5. 全体を小文字化する。
6. 先頭・末尾のアンダースコアは除去する。
7. 長すぎる場合（例: 100文字以上）は適当な長さで切る。
例: `https://ja.wikipedia.org/wiki/マンチェスター・シティFC`
→ `ja_wikipedia_org__wiki_manchester_city_fc` (日本語が含まれる場合はそのまま、またはローマ字変換推奨だが、FSが許容するならそのままでも可。基本はURLエンコード部分をデコードして可読性を高める)
