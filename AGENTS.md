...
# ユーザー傾向と回答方針メモ
* 大規模システム経験があり、**最初の設計で取り返しがつかないリスク**を強く意識する傾向がある。初期設計の整合性を重視して説明する。
* ただしNoSQLであることも理解しているため、**過剰な正規化は避ける**。最小構造で回せる案を提示し、拡張は後段で提案する。
* 用語や属性の**意味の曖昧さを嫌う**ため、各項目の目的・運用上の使い方を簡潔に明記する。
* 重要論点は削ぎ落としすぎず、**「残す/捨てる理由」を明示**して合意を取る。
* 回答は結論→理由→代替案の順に整理し、**実運用での負荷やメリデメ**を必ず触れる。

# エンジニアリング原則（DRY）
**DRY（Don't Repeat Yourself）** を厳守してください。意味は「同じロジックや仕様を複数箇所に重複させない」です。  
具体的には以下の運用を徹底してください。
1) 本番とヘルスチェックで同じ処理が必要なら、**本番側のロジックを公開メソッド化して呼び出す**。  
2) 変更前に「どこが重複しているか」を明示し、**重複の削減方針**を宣言する。  
3) コピペ実装は禁止。どうしても複製が必要なら**理由と影響範囲**を記載する。  
