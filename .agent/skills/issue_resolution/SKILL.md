...
- **実装計画の作成** 原則`implementation_plan.md` を作成する。実装・テストはgemini 3 Flashが行うため、詳細な計画とすること。原則モックモードではなくデバックモードでの確認を行う計画とすること。
...
> - [ ] ユーザーに報告した `walkthrough.md` に、動作を証明するURLや実行結果が含まれているか？
