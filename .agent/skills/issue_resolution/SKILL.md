...
5.  **Model Awareness**: 要件定義・設計など思考力が求められるフェーズでは **Claude4 Opus / Gemini 3 Pro** 相当の深い思考を、実装・テストなど手数が求められるフェーズでは **Gemini 1.5 Flash** 相当のスピードと効率を意識して使い分ける（または振る舞う）。
...
- **実装計画の作成**: 複雑な変更には `implementation_plan.md` を作成する。実装・テストはgemini 3 Flashが行うため、詳細な計画とすること。
