...
def print_results(label: str, results: List[Dict], keywords: List[str]):
    ...
    print("=" * 80)
    for r in results:
        flags = flag_keywords(f"{r.get('title', '')} {r.get('description', '')}", keywords)
...
def run_training_queries(yt: YouTubeService, teams: List[str], kickoff_utc: datetime, max_results: int):
    """練習動画クエリをテスト"""
    print("\n" + "=" * 80)
...
def run_player_queries(
    ...
    for player in players:
        ...
        print_results(f"PLAYER | {player} ({team})", result["kept"], PLAYER_FLAG_KEYWORDS)
...
def main():
    ...
    parser.add_argument("--kickoff-jst", default=DEFAULT_KICKOFF_JST, help="Kickoff time in JST (YYYY-MM-DD HH:MM)")
    parser.add_argument("--players", nargs="*", default=DEFAULT_PLAYERS, help="Player names to search")
    parser.add_argument("--max-results", type=int, default=5, help="Max results per query")
    parser.add_argument("--no-filter", dest="filter", action="store_false", help="Disable player post-filter")
    parser.set_defaults(filter=True)
    parser.add_argument("--mode", choices=["training", "player", "all"], default="all", help="Query mode")
    ...
    if args.mode in ("player", "all"):
        ...
        run_player_queries(yt, args.players, args.home, kickoff_utc, args.max_results, args.filter)
