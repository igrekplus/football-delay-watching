...
from settings.channels import is_trusted_channel
...
def print_video(idx: int, video: dict, status: str, reason: str = ""):
    ...
    print(
        f"   Channel: {channel} ({trusted_mark})"
        if trusted_mark
...
def cmd_query(args):
    ...
    print(
        f"PARAMS: published_after={published_after.strftime('%Y-%m-%d')}, "
...
def cmd_category(args):
    ...
    if args.kickoff_jst:
        ...
        # デフォルト: 翌日00:00 JST
        tomorrow = datetime.now(jst).replace(
    ...
    if category == "press_conference" and not args.team:
        ...
        if not query_vars["team_name"]:
            print(
                "ERROR: --team or --home/--away is required for 'press_conference' category"
            )
            return 1
    ...
    print_summary(
        len(kept), len(removed), "クエリを変更するには settings/search_specs.py を編集"
    )
...
def cmd_filter(args):
    ...
    videos = (
        ...
        else data.get("kept", []) + data.get("removed", [])
    )
...
def main():
    ...
    query_parser.add_argument(
        "--days-before", type=int, default=7, help="何日前から検索 (default: 7)"
    )
    query_parser.add_argument(
        "--max-results", type=int, default=20, help="表示件数 (default: 20)"
    )
    query_parser.add_argument(
        "--no-filter", action="store_true", help="フィルタを適用しない"
    )

    # category サブコマンド
    cat_parser = subparsers.add_parser(
        "category", help="カテゴリ別の自動クエリをテスト"
    )
    cat_parser.add_argument(
        "category", choices=list(YOUTUBE_SEARCH_SPECS.keys()), help="検索カテゴリ"
    )
    ...
    cat_parser.add_argument(
        "--show-removed", action="store_true", help="除外動画も表示"
    )
