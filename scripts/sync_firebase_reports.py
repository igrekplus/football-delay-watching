...
def merge_manifests(local: dict, remote: dict) -> dict:
    ...
    """
    # 新構造のベース
    merged = {"reports_by_date": {}, "legacy_reports": []}
...
def sync_reports() -> None:
    ...
    remote_total = sum(len(d.get("matches", [])) for d in remote_by_date.values())
    ...
    # 新構造対応のログ出力
    match_count = sum(
        len(d.get("matches", [])) for d in merged.get("reports_by_date", {}).values()
    )
