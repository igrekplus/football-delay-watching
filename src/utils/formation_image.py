...
class FormationImageGenerator:
    ...
    def _create_pitch(self) -> Image.Image:
        ...
        draw.line(
            [(margin, center_y), (self.width - margin, center_y)],
            fill=LINE_COLOR,
            width=2,
        )
        # Center circle
        ...
        draw.rectangle(
            [
                self.width // 2 - penalty_width // 2,
                margin,
                self.width // 2 + penalty_width // 2,
                margin + penalty_height,
            ],
            outline=LINE_COLOR,
            width=2,
        )
        ...
        draw.rectangle(
            [
                self.width // 2 - penalty_width // 2,
                self.height - margin - penalty_height,
                self.width // 2 + penalty_width // 2,
                self.height - margin,
            ],
            outline=LINE_COLOR,
    ...
    def _get_layout(self, formation: str) -> list[tuple[float, int]] | None:
    ...
    def _draw_title(self, draw: ImageDraw.Draw, title: str, is_home: bool):
        ...
        draw.rectangle(
            [x - 8, y - 4, x + text_width + 8, y + text_height + 4], fill=color
        )
...
def get_formation_layout_data(
    ...
    if not layout:
        logger.warning(f"Unknown formation: {formation}, using 4-4-2")
        layout = FORMATION_LAYOUTS["4-4-2"]
    if player_profile_urls is None:
        player_profile_urls = {}
...
def generate_formation_image(
    ...
    result = generator.generate(
        formation, players, team_name, is_home, output_path, player_numbers
    )
