...
class TributeGenerator:
    ...
    def generate_former_club_trivia(self, match: MatchAggregate):
        ...
        match.facts.former_club_trivia = re.sub(r'\s*\[\d+(?:,\s*\d+)*\]', '', raw_trivia) if raw_trivia else ""
        
        if match.facts.former_club_trivia:
            logger.info(f"Generated former club trivia for {match.core.home_team} vs {match.core.away_team}")

    def _detect_same_country_matchups(self, match: MatchAggregate) -> List[Dict[str, Any]]:
        """同国対決を検出"""
        home_players = match.facts.home_lineup + match.facts.home_bench
        away_players = match.facts.away_lineup + match.facts.away_bench
