...
class MatchAggregate:
    ...
    def kickoff_at_utc(self, value: datetime | None):
    ...
    def get_report_filename(self, generation_datetime: str) -> str:
        ...
        filename = (
            f"{match_date}_{home_normalized}_vs_{away_normalized}_{generation_datetime}"
        )
        return filename
...
class MatchData:
    ...
    # 負傷者・出場停止情報
    injuries_list: list = (
        None  # [{"name": "Player", "team": "Team", "reason": "Injury"}, ...]
    )
    ...
    def __post_init__(self):
        ...
        if self.away_lineup is None:
            self.away_lineup = []
        if self.home_bench is None:
            self.home_bench = []
        if self.away_bench is None:
            self.away_bench = []
        if self.player_nationalities is None:
            self.player_nationalities = {}
        if self.player_numbers is None:
            self.player_numbers = {}
        if self.player_photos is None:
            self.player_photos = {}
        if self.player_birthdates is None:
            self.player_birthdates = {}
