...
class MatchRanker:
    ...
    def assign_rank(self, match: MatchAggregate) -> None:
        ...
        if any(
            t in match.core.home_team or t in match.core.away_team
            for t in config.S_RANK_TEAMS
        ):
            match.core.rank = "S"
            logger.info(
                f"Assigned S to {match.core.home_team} vs {match.core.away_team}"
            )
            return
        ...
        if any(
            t in match.core.home_team or t in match.core.away_team
            for t in config.A_RANK_TEAMS
        ):
            match.core.rank = "A"
            logger.info(
                f"Assigned A to {match.core.home_team} vs {match.core.away_team}"
            )
            return
        ...
        if any(
            ...
            match.core.rank = "A"
            logger.info(
                f"Assigned A (Japanese player) to {match.core.home_team} vs {match.core.away_team}"
            )
            return
