...
class YouTubeSearchClient:
    ...
    def _read_cache(self, cache_path: str) -> list[dict] | None:
    ...
    def search(
        ...
        try:
            ...
            if published_before:
                params["publishedBefore"] = published_before.strftime(
                    "%Y-%m-%dT%H:%M:%SZ"
                )
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"YouTube API: '{query}' -> {len(results)} results (API calls: {self.api_call_count})"
                )
