...
class NewsService:
    ...
    def process_news(self, matches: list[MatchAggregate]):
        ...
        for match in matches:
            if match.core.is_target:
                logger.info(
                    f"Processing news for {match.core.home_team} vs {match.core.away_team}"
                )
                ...
                if raw_summary and not config.USE_MOCK_DATA:
                    spoiler_result = self.llm.check_spoiler(
                        raw_summary, match.core.home_team, match.core.away_team
                    )
                    is_safe, reason, unsafe_evidence = self._unpack_spoiler_result(
                        spoiler_result
                    )
                    if not is_safe and unsafe_evidence:
                        logger.warning(
                            "[SPOILER CHECK] action=news_summary_hidden "
                            "fixture_id=%s home_team=%s away_team=%s reason=%s "
                            "unsafe_evidence=%s summary_preview=%s",
                            match.core.id,
                            match.core.home_team,
                            match.core.away_team,
                            reason,
                            unsafe_evidence,
                            raw_summary[:180].replace("\n", " "),
                        )
                        ...
                        logger.warning(
                            "[SPOILER CHECK] action=inconsistent_verdict "
                            "fixture_id=%s home_team=%s away_team=%s reason=%s",
                            match.core.id,
                            match.core.home_team,
                            match.core.away_team,
    ...
    @staticmethod
    def _unpack_spoiler_result(
        result: Any,
    ) -> tuple[bool, str, list[dict[str, str]]]:
        """旧2要素タプルのモックと、新3要素タプルの両方を受ける。"""
        if not isinstance(result, tuple):
            return True, "判定スキップ（不正な戻り値）", []

        if len(result) >= 3:
            is_safe, reason, unsafe_evidence = result[:3]
            if isinstance(unsafe_evidence, list):
                return bool(is_safe), str(reason or ""), unsafe_evidence
            return bool(is_safe), str(reason or ""), []
        ...
        if len(result) == 2:
            ...
            return (
                ...
                str(reason or ""),
                (
                    [{"type": "legacy", "quote": str(reason or "")}]
                    if not is_safe
                    else []
                ),
            )
    ...
    def _process_interviews(self, match: MatchAggregate):
        ...
        for is_home in [True, False]:
            ...
            manager_name = (
                match.facts.home_manager if is_home else match.facts.away_manager
            )
            opponent_manager_name = (
                match.facts.away_manager if is_home else match.facts.home_manager
            )
