...
### 5. タスク範囲の厳守 (Strict Scope Limitation)
- **原則**: ユーザーから依頼された特定のIssueやタスクのみに集中して取り組むこと。
- **禁止事項**: 依頼されていない別のIssueの計画や実装を、明示的な指示がない限り開始してはならない。

### 6. プロンプト管理 (Prompt Management)
- **原則**: すべてのLLMプロンプトはハードコードせず、`settings/prompts/` 以下のMarkdownファイルとして外部化する。
- **管理方法**: `settings/gemini_prompts.py` でメタデータ（ラベル、Grounding使用有無等）を管理し、`build_prompt` 関数を通じて呼び出す。
- **目的**: プロンプトの可視化、再利用、およびチューニングを容易にするため。
