...
def verify_api_client():
    ...
    if config.USE_MOCK_DATA:
        logger.info(
            "Skipping API calls in Mock Mode (enable DEBUG_MODE=True USE_MOCK_DATA=False to test real API)"
        )
        return
...
def verify_domain_logic():
    ...
    for m in selected:
        logger.info(
            f" - {m.home_team} vs {m.away_team} (Target: {m.is_target}, Reason: {m.selection_reason})"
        )
