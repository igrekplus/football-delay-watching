...
def test_rank_filter():
    ...
    match_s = MatchAggregate(
        core=MatchCore(id="1", home_team="Team S", away_team="Away S", competition="C", kickoff_jst="T", kickoff_local="T", rank="S", is_target=True),
        facts=MatchFacts()
    )
    
    # 2. Aランクの試合
    match_a = MatchAggregate(
        core=MatchCore(id="2", home_team="Team A", away_team="Away A", competition="C", kickoff_jst="T", kickoff_local="T", rank="A", is_target=True),
        facts=MatchFacts()
    )
    
    # 3. Absoluteランクの試合
    match_abs = MatchAggregate(
        core=MatchCore(id="3", home_team="Team Abs", away_team="Away Abs", competition="C", kickoff_jst="T", kickoff_local="T", rank="Absolute", is_target=True),
        facts=MatchFacts()
    )
    
    # 4. Bランクの試合（スキップされるべき）
    match_b = MatchAggregate(
        core=MatchCore(id="4", home_team="Team B", away_team="Away B", competition="C", kickoff_jst="T", kickoff_local="T", rank="B", is_target=True),
        facts=MatchFacts()
    )
    
    # 5. ランクなし（スキップされるべき）
    match_none = MatchAggregate(
        core=MatchCore(id="5", home_team="Team None", away_team="Away None", competition="C", kickoff_jst="T", kickoff_local="T", rank="", is_target=True),
        facts=MatchFacts()
    )
    ...
    def mock_get_videos(match):
        nonlocal call_count
        call_count += 1
        print(f"  [CALL] get_videos_for_match called for {match.core.home_team} (rank={match.core.rank})")
