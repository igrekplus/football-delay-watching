...
def test_cache_delete():
    ...
    assert store.exists(test_path)
    logger.info(f"Cache created: {test_path}")

    deleted = store.delete(test_path)
    assert deleted
    assert not store.exists(test_path)
    logger.info("Cache deleted successfully via CacheStore")
    ...
    assert store.exists(cache_path)
    logger.info(f"Dummy cache created at: {cache_path}")

    # Delete via client
    deleted = client.delete_cache(url, params)
    assert deleted
    assert not store.exists(cache_path)
    logger.info("Cache deleted successfully via CachingHttpClient")
